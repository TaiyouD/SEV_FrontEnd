    ...
    filterData() {
      ...
      if (this.selectedDate && this.selectedDate !== "All Dates") {
        if (this.selectedDate === "Current") {
          ...
          filteredData = filteredData.filter(event => new Date(event.event.date) >= today && new Date(event.event.date) < new Date(today.getTime() + 24 * 60 * 60 * 1000));
        } else if (this.selectedDate === "Past") {
          const now = new Date();
          const today = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
          filteredData = filteredData.filter(event => new Date(event.event.date) < today);
        } else if (this.selectedDate === "Upcoming ") {
          const now = new Date();
          const tomorrow = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + 1));
          filteredData = filteredData.filter(event => new Date(event.event.date) >= tomorrow);
        }
      }
